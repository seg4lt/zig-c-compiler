...
fn scanLabelOnStmt(s: Self, stmt: *Ast.Stmt, scope_labels: *ScopeLabels) SemaError!void {
    switch (stmt.*) {
        .Switch => |*switch_stmt| {
            ...
            try s.scanLabelOnBlockItem(&body, scope_labels);
        },
...
fn scanLabelOnExpr(s: Self, expr: *Ast.Expr, scope_labels: *ScopeLabels) SemaError!void {
    switch (expr.*) {
        .Cast => |cast_expr| try s.scanLabelOnExpr(cast_expr.expr, scope_labels),
    ...
    switch (stmt.*) {
        .Switch => |*switch_stmt| {
            for (switch_stmt.body.items) |item| {
                switch (item.*) {
                    .Decl => {}, // noop
                    .Stmt => |st| try s.resolveGotoOnStmt(st, goto_labels, scope_labels),
                }
            }
        },
        ...
        .Case, .Default, .Break, .Continue, .Null => {}, // noop
    }
}
fn resolveGotoOnExpr(s: Self, expr: *Ast.Expr, goto_labels: *ScopeLabels, scope_labels: *ScopeLabels) SemaError!void {
    switch (expr.*) {
        .Cast => |cast_expr| try s.resolveGotoOnExpr(cast_expr.expr, goto_labels, scope_labels),
